...
If, at least, one of the last *K* elements of the vector **z** is non-zero, the system has no solutions. Otherwise, any vector $\mathbf{w} = c_1 * [w_1^{(1)}, \dots, w_{N-K}^{(1)}, 1, 0, \dots, 0]^T + c_2 * [w_1^{(2)}, \dots, w_{N-K}^{(2)}, 0, 1, \dots, 0]^T + \dots + c_K * [w_1^{(K)}, \dots, w_{N-K}^{(K)}, 0, 0, \dots, 1]^T$ is a solution of the system, where each vector $\mathbf{w}^{(j)}$ is the solution of the matrix equation $\mathbf{U}_{(N-K)*(N-K)} * \mathbf{w}^{(j)} = [z_1, z_2, \dots, z_{N-K}]^T - \mathbf{b}^{(j)}$, where $\mathbf{b}^{(j)}$ is the *j*-th column of the $\mathbf{B}_{K * (N-K)}$ sub-matrix.
