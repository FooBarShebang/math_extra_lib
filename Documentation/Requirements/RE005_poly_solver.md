...
* Accept three arguments:
  * a real number value $x_{min}$ - the smallest x-value (left boundary)
  * a real number value $x_{max}$ - the largest x-value (right boundary)
  * a sequence of real number values $\{y_i\}$  - values of a function at the nodes of the equidistant the x-coordinates grid $\{x_i\}$ with N points from and incluing $x_{min}$ to and including $x_{max}$
* Calculate the interpolating polynomial of the degree $0 \leq K \leq N-1$ using Berstein base as $\sum_{i=0}^{N-1}{y_i * b_{i,N-1}(Q(x_i))}$, where *N* is the number of the points, where $Q(x)$ is the 1-st degree (linear) polynomial describing the mapping of the interval $[x_{min}, x_{max}] \rightarrow [0, 1]$
* Return calculated polynomial as an instance of the **Polynomial** class for *K* > 0 and as a real number for *K* = 0
...
**Description:** The functions generating interpolation polynomial on an arbitrary x-grid (Lagrange, Legendre, Chebyshev) raise an exception compatible with TypeError if the passed argument is not a sequence of 2-element sub-sequences of real numbers
...
**Description:** The functions generating interpolation polynomial on an arbitrary x-grid (Lagrange, Legendre, Chebyshev) raise an exception compatible with ValueError if there is, at least, 1 not unique value amongst the first elements of the sub-sequences (x-grid with non-unique values)
...
**Requirement ID:** REQ-AMW-590

**Title:** Berstien interpolation polynomial - improper argument type

**Description:** The function generating Berstein interpolation polynomial raises an exception compatible with TypeError if:

* the first argument (min x) is not a real number - not of the types **int** or **float**
* the second argument (max x) is not a real number - not of the types **int** or **float**
* the third argument (y-values) is not a flat sequence of real numbers

**Verification Method:** T

---

**Requirement ID:** REQ-AMW-591

**Title:** Berstein interpolation polynomial - improper x values

**Description:** The function generating generating Berstein interpolation polynomial raises an exception compatible with ValueError if:

* the second argument (max x) is not larger than the first (min x)
* the length of the third argument (y-values) is less than 2
